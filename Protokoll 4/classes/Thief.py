...
class Thief(Character):
    """
    Klasse der Heldenklasse Thief. Der Name der Klasse ist als globales Klassenattribut gesetzt.
    """
    __class_name = "Thief"
    ...
    def sneak_attack(self):
        """
        Der Spezialangriff des Diebes.
        :return: self._Character__sleight_of_hand * 2 or return False
        """
        self.set_special_attack(1)
        die_a = randint(1,20)
        die_b = randint(1,20)

        if die_a <= self._Character__intuition and die_b <= self._Character__dexterity:
            return self._Character__sleight_of_hand * 2
        else:
