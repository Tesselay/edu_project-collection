    ...
    def level_check(self):
        ...
        for items in range(0, len(level_exp)):
            if self.__experience >= level_exp[items] and self.__level == items:                                         # Der Level-Up wird nur ausgeführt, sofern die Erfahrung höher als der benötigte Wert sind und das Level dem Index
                ...
                self.__level += 1
                self.level_up()
            elif self.__experience <= level_exp[items] and self.__level == items:
                print("Your group needs {} experience for the next level.".format(level_exp[items]-self.__experience))
            ...
            while error_loop:                                                                                           # While-Schleife die solange wiederholt wird, bis ein Attribut vergeben wird.
                try:                                                                                                    # Da der folgende Input in einen Integer verwandelt wird, wird ein Exceptionhandling eingebaut.
                    ...
                    if attribute == 1:
                        ...
                        self.__heroes[items].set_charisma(self.__heroes[items].get_charisma() + 1)
                        error_loop = False
                    elif attribute == 6:
                        self.__heroes[items].set_stamina(self.__heroes[items].get_stamina() + 1)
                        error_loop = False
                    elif attribute == 7:
                        self.__heroes[items].set_intuition(self.__heroes[items].get_intuition() + 1)
                        error_loop = False
                    else:
